...
impl VmmapOps for Vmmap {
    fn add_entry(&mut self, vmmap_entry_ref: VmmapEntry) {
        let _ = self.entries.insert_strict(
            ...
            ie(vmmap_entry_ref.page_num, vmmap_entry_ref.page_num + vmmap_entry_ref.npages),
            vmmap_entry_ref
        );
    }
    ...
    fn remove_entry(&mut self, page_num: u32, npages: u32) -> Result<(), io::Error> {
        self.update(page_num, npages, 0, 0, 0, MemoryBackingType::None, true, 0, 0, 0)
    }
    ...
    fn update(
        ...
        if npages == 0 {
            return Err(
                io::Error::new(io::ErrorKind::InvalidInput, "Number of pages cannot be zero")
            );
        }
        ...
        let _ = self.entries.insert_overwrite(
            ie(new_region_start_page, new_region_end_page),
            new_entry
        );
        ...
        if remove {
            ...
            let _ = self.entries.remove_overlapping(ie(new_region_start_page, new_region_end_page));
        }
    ...
    fn change_prot(&mut self, page_num: u32, npages: u32, new_prot: i32) {
        ...
        for (overlap_interval, entry) in self.entries.overlapping_mut(
            ie(new_region_start_page, new_region_end_page)
        ) {
    ...
    fn find_page_iter_mut(
        ...
        if let Some(last_entry) = self.last_entry() {
            self.entries.overlapping_mut(ie(page_num, last_entry.0.end()))
        } else {
            // Return an empty iterator if no last_entry
    ...
    fn find_space(&self, npages: u32) -> Option<Interval<u32>> {
        ...
        if start == None || end == None {
            ...
            for gap in self.entries.gaps_trimmed(ie(start_unwrapped, end_unwrapped)) {
    ...
    fn find_map_space(&self, num_pages: u32, pages_per_map: u32) -> Option<Interval<u32>> {
        ...
        if start == None || end == None {
            ...
            let rounded_num_pages = self.round_page_num_up_to_map_multiple(
                num_pages,
                pages_per_map
            );

            for gap in self.entries.gaps_trimmed(ie(start_unwrapped, end_unwrapped)) {
                let aligned_start_page = self.trunc_page_num_down_to_map_multiple(
                    gap.start(),
                    pages_per_map
                );
                let aligned_end_page = self.round_page_num_up_to_map_multiple(
                    gap.end(),
                    pages_per_map
                );
...
mod tests {
    ...
    fn test_change_prot_within_maxprot() {
        ...
        assert_eq!(
            ...
            "Protection not changed as expected"
        );
    ...
    fn test_change_prot_beyond_maxprot() {
        ...
        let invalid_change = vmmap.change_prot(
            5,
            5,
    ...
    fn test_change_prot_non_existent_range() {
        ...
        assert!(
            non_existent_change.is_err(),
            "Changing protection for non-existent range should fail"
        );
    }
    ...
    fn test_change_prot_across_multiple_entries() {
        ...
        assert_eq!(
            vmmap.entries.get_at_point(8).unwrap().prot,
            ProtFlags::READ | ProtFlags::WRITE,
            "Protection not changed as expected for first affected entry"
        );
        assert_eq!(
            vmmap.entries.get_at_point(10).unwrap().prot,
            ProtFlags::READ | ProtFlags::WRITE,
            "Protection not changed as expected for second affected entry"
        );
    }
    ...
    fn test_remove_portion_of_entry() {
        ...
        assert_eq!(
            vmmap.entries.get_at_point(0).unwrap().npages,
            5,
            "First entry should be truncated"
        );
        assert_eq!(
            vmmap.entries.get_at_point(15).unwrap().npages,
            5,
    ...
    fn test_remove_across_multiple_entries() {
        ...
        assert_eq!(
            vmmap.entries.len(),
            2,
            "Expected 2 entries after removal across multiple entries"
        );
        assert_eq!(
            vmmap.entries.get_at_point(0).unwrap().npages,
            2,
        ...
        assert_eq!(
            vmmap.entries.get_at_point(20).unwrap().npages,
            5,
            "Last entry should remain unchanged"
        );
    }
    ...
    fn test_check_existing_mapping_large_values() {
        ...
        assert_eq!(
            result.unwrap(),
            (1_000_000, 1000),
            "Should return correct existing mapping range"
        );
        ...
        assert!(
            result.is_none(),
            "Should not detect existing mapping for non-overlapping large range"
        );
    }
    ...
    fn test_check_addr_mapping_large_values() {
        ...
        assert_eq!(
            result.unwrap().0,
            1_000_000,
            "Should return correct start page for large entry"
        );
        assert_eq!(
            result.unwrap().1,
            1000,
            "Should return correct number of pages for large entry"
        );
    ...
    fn test_find_page_mut() {
        ...
        assert_eq!(
            vmmap.entries.get_at_point(5).unwrap().prot,
            ProtFlags::WRITE,
    ...
    fn test_find_page_iter() {
        ...
        assert_eq!(
            iter.next().unwrap().prot,
            ProtFlags::READ,
            "First entry should have READ protection"
        );
        assert_eq!(
            iter.next().unwrap().prot,
            ProtFlags::WRITE,
            "Second entry should have WRITE protection"
        );
    ...
    fn test_find_page_iter_mut() {
        ...
        assert_eq!(
            vmmap.entries.get_at_point(0).unwrap().prot,
            ProtFlags::READ | ProtFlags::WRITE,
            "First entry should be modified"
        );
        assert_eq!(
            vmmap.entries.get_at_point(20).unwrap().prot,
            ProtFlags::READ,
            "Second entry should be modified"
        );
